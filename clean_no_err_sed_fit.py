...
os.chdir('/media/intern2/DD82-B92F/RESEARCH_FALL_2022/FILTERS/trimmed_transmissivity_curves/')		#Go to directory where you keep transmissivity curve data

J_2MASS_lams = np.loadtxt('J_2MASS.TRM')[:,0] * unit['AA']						#Column 0 of transmissivity data is wavelengths (in angstroms)
J_2MASS_trans = np.loadtxt('J_2MASS.TRM')[:,1]								#Column 1 of transmissivity data is trasmission percentage in decimal format (eg 25% would show up as 0.25 in column 1)
H_2MASS_lams = np.loadtxt('H_2MASS.TRM')[:,0] * unit['AA']						#All transmissivity curves have been trimmed to include only those (wavelength,transmission) data points with transmissivity > 0.0002
...
J_2MASS_filter = Filter(J_2MASS_lams,J_2MASS_trans,unit='Angstrom')					#Create filters using pyphot module... these filters are to be applied to model spectra with flux units of erg/cm^2/s/angstrom (Flam). The Johnson filters - by default - are to be applied to models with flux in erg/cm^2/s/Hz (Fnu) so need to convert these to Flam using dtype='energy'
...
#lib = pyphot.get_library()				#These commented out filters are the built-in filters of the pyphot module... I have them in here just in case you feel the above steps of creating a filter from (wavelength vs transmissivity) raw data isn't working correctly
...
#nuv_galex_filter = lib['GALEX_NUV']
						...
						#READ IN TEMP AND LOGG
os.chdir('/media/intern2/DD82-B92F/RESEARCH_FALL_2022/')				#Go to directory where independetly determined Teff and logg (for many stars) are stored

#temp = np.loadtxt("Roy_teff_logg_real_names.txt",usecols=0)[2]			#read in teff, logg, and (their respective errors) for star_name
#temp_err = np.loadtxt("Roy_teff_logg_real_names.txt",usecols=1)[2]
#logg = np.loadtxt("Roy_teff_logg_real_names.txt",usecols=2)[2]
#logg_err = np.loadtxt("Roy_teff_logg_real_names.txt",usecols=3)[2]

os.chdir('/media/intern2/DD82-B92F/RESEARCH_FALL_2022/stitched_blackbody_binned_models/')		#Go to directory where binned (all evaluated at same wavelengths) and stitched (blackbody spectrum appended at long wavelengths) model spectra are stored
star_name = input("Star Name:")
temp = 31041.0										#gd934 has been my test star, these are correct teff and logg values for gd934 with an error budget of 1%
temp_err = 0.01*temp
logg = 5.52
logg_err = 0.01*logg
rv = 3.1										#Average Milky Way value with a 10% error budget (Rv correlates to level UV extinction)
rv_err = 0.1*rv
model_list = os.listdir('/media/intern2/DD82-B92F/RESEARCH_FALL_2022/stitched_blackbody_binned_models/')
numbered_model_list = []
temp_to_save = temp									# ***_to_save is purley to save temp, gravity, reddening parameters, and distance in star_name_observed_params.txt after completion of fitting method
grav_to_save = logg
...
for model in model_list:
	numbered_model_list.append(model[17]+model[18]+model[19]+model[21]+model[22]+model[23])			#Example model name is "binned_stitched_t160g450sdbv00.txt" so model[17]+model[18]+model[19] gives teff = 16,000K and model[21]+model[22]+model[23] gives logg = 4.50 dex


Temperature_values = []									#Initialize lists which will be added to within Monte Carlo Simulation
...
G = 6.6743*10**(-11)							#gravitational constant in m^3/kg/s^2
				...
				#GET COORDINATES FROM STAR NAME USING SIMBAD
dec_hms = Simbad.query_object(star_name)['DEC']					#gather declination and right ascension in hours/minutes/seconds... and convert both to degrees
...
print(co)
                                ...
                                #E(B-V) ASTROQUERY
dust_radius = 2*u.deg
dust_table = IrsaDust.get_query_table(co, radius=dust_radius, section='ebv')		#do a cone search with radius of 2 degrees... remember this is a measure of the dispersion of interstellar dust along your line of sight and so a small search radius is not necessary (as it would be if you were looking for a particular star)
...
ebv_err_to_save = ebv_err
				...
				#Rv ASTROQUERY
#Rv_table = dustmaps.(co, radius=dust_radius, section='ebv')		#If you can figure out the correct import for "dustmaps" (I was having issues doing so) this might be a start to getting Rv values through astroquery instead of setting Rv = 3.1 with a 10% error budget


				#SDSS ASTROQUERY
sdss_radius = 0.00005   #in degrees
sdss_query = """ SELECT u,err_u,g,err_g,r,err_r,i,err_i,z,err_z FROM PhotoObj WHERE ra>{ra_min} AND ra<{ra_max} AND dec>{dec_min} AND dec<{dec_max} """.format(ra_min=ra_deg-sdss_radius,ra_max=ra_deg+sdss_radius,dec_min=dec_deg-sdss_radius,dec_max=dec_deg+sdss_radius)					#this is a box search NOT a cone search... so sdss_radius is really height and width of box, centered at (ra,dec)
sdss_query_standard = """ SELECT u FROM PhotoObj WHERE ra>18.5775 and ra<18.5776 and dec>-0.8205 and dec<-0.8204 """			#I KNOW what type of output I get for sdss_query_standard... I can check all other queries against this standard to make sure the gathered data is the correct type
sdss_job = SDSS.query_sql(sdss_query,data_release=18)
sdss_job_standard = SDSS.query_sql(sdss_query_standard,data_release=18)

if type(sdss_job) == type(sdss_job_standard):					#here is where I check the type of all other queries against the standard
	...
	for num in range(0,len(u_mag)):						#Many times there will be multiple results for a single query, even with the small sdss_radius... this for loop and next two lines finds the average magnitude and error for each SDSS filter
	...
	u_zp_Jy_flux,g_zp_Jy_flux,r_zp_Jy_flux,i_zp_Jy_flux,z_zp_Jy_flux = 3767,3631,3631,3631,3565			#convert filter magnitudes into flux units (and magnitude errors to flux errors)
...
if type(sdss_job) != type(sdss_job_standard):					#When the query type does not match that of the standard, that means star_name does not have SDSS data
...
sdss_result = SDSS.query_region(co,spectro=True)				#not extremely useful, but checking to see if star_name has a spectrum in SDSS
...
if sdss_length == 35:                                                   #sdss_length=35 corresponds to an astropytable which means sdss_result != None
        ...
        #plt.show()							#If there is a spectrum, you can plot it by un-commenting the 4 plt. lines to the left
else:
...
gaia_job = Gaia.launch_job(gaia_query)						#same idea as SDSS here... looking for distance instead of photometry though
gaia_results = gaia_job.get_results()
if len(gaia_results) !=0 and gaia_results[0]['parallax'] >= 0:			#some parallax measurements are negative in the GAIA database, but we need to filter those out as they are not useful
	print('GAIA:')
	parallax_to_sum,parallax_err_to_sum = [],[]
	parallax,parallax_err = gaia_results['parallax'],gaia_results['parallax_error']

	for val in range(0,len(parallax)):					#this for loop and the next two lines find the average parallax and error for star_name
	...
	distance = 1/(np.tan(avg_parallax/1000))				#convert parallax (in arcseconds) to distance (in parsecs)
	...
	distance = 'NA'
	distance_err = 'NA'
	sys.exit("NO DISTANCE MEASUREMENT IN GAIA")		#terminates run here if star_name does not have a distance measurement in GAIA (SED fitting method requires a distance measurement)
...
wise_job = Irsa.query_region(co, catalog='allwise_p3as_psd', spatial='Cone', radius=wise_radius)			#cone search for WISE photometry
...
for entry in range(0,len(W1_mag)):						#there are probably multiple results from the cone search... here we set up lists consisting of the multiple entries for filter magnitudes and their errors
...
if len(W1_to_sum)>=1:								#average magnitudes and errors for each filter
...
if type(wise_list[0]) == np.float64:						#convert magnitudes and errors to Janskys
...
if type(Jy_list[0]) == np.float64:						#convert Janskys to erg/cm^2/s/angstrom
...
print('W4 error erg:', erg_error_list[3])
                        ...
                        #GALEX ASTROQUERY
galex_region = Catalogs.query_region(co,radius=0.002,catalog="galex")			#another cone search
...
if len(galex_region)!=0:
	sum_fuv_flux,sum_fuv_flux_err,sum_nuv_flux,sum_nuv_flux_err = [],[],[],[]

	for n in range(0,len(galex_fuv_flux)):				#GALEX provides their photometry measurements in flux units as well as in magnitudes... we want flux units
	...
	if len(sum_fuv_flux)!=0:				#average flux converted photometry and errors (in microJanskys)
	...
	if type(avg_galex_fuv_flux) != str:				#convert microJanskys to erg/cm^2/s/angstrom
	...
	if type(avg_galex_fuv_flux_err) != str:
		fuv_err_micro_jansky = avg_galex_fuv_flux + avg_galex_fuv_flux_err
...
vizier_result = Vizier.query_region(co,radius=vizier_radius,catalog='II/246')				#this is a database which houses 2MASS photometry for many stars... this is NOT directly linked to the 2MASS mission
vizier_result_str = str(Vizier.query_region(co,radius=vizier_radius,catalog='II/246'))			#There is no way to directly access the original 2MASS database through astroquery, so we get the 2MASS photometry for star_name (if present) from this database
if vizier_result_str[0] != 'E':
	...
	for entry in Jmag_list:							#queries to this database return information of type numpy.float32, need to convert to numpy.float64 for math later on in code
	...
	Kmags_to_sum = []
	Kmag_errs_to_sum = []
	for i in range(0,len(float64_Jmag_list)):					#get magnitudes and errors ready to be averaged
	...
	if len(Jmags_to_sum)!=0:							#average the magnitudes and errors
	...
	if type(avg_Jmag)!=str:								#convert magnitudes and errors to erg/cm^2/s/angstrom
...
if  err_u_flux_avg_erg == 'NA' or err_g_flux_avg_erg == 'NA':					#we only want to use Johnson photometry if we do NOT have sdss data... we don't use sdss data if no error is given with its measurement
	Vizier.ROW_LIMIT = -1
	Vizier.columns = ['all']
	Johnson_result = Vizier.query_object(star_name,catalog='J/A+A/600/A50')			#this is a database of sdB/sdO type stars curated by Stephan Geier (he is prevalent in the subdwarf stars community)
	Johnson_result_str = str(Vizier.query_object(star_name,catalog='J/A+A/600/A50'))
	if Johnson_result_str[0] != 'E':							#gather magnitudes and errors... convert from type numpy.float32 to type numpy.float64
		...
		if type(check_if_data_present[0]) == np.float32:
			...
			Johnson_B_flux_erg = 6.32*(10**-9)*(10**(Johnson_B_mag/-2.512))			#convert magnitudes and errors to erg/cm^2/s/angstrom
...
for i in range(0,10):										#number of models to be generated
	rand_err_for_Teff = (random.uniform(-1,1))*inputs[2]					#randomly pick specific value for each parameter's error from within each individual error budget
	rand_err_for_LOGg = (random.uniform(-1,1))*inputs[4]
	rand_err_for_EBV = (random.uniform(-1,1))*inputs[6]
	rand_err_for_Rv = (random.uniform(-1,1))*inputs[32]
	if type(inputs[8]) == np.float64:
		rand_err_for_sdss_u_flux = (random.uniform(-1,1))*inputs[8]
	else:
		rand_err_for_sdss_u_flux = 'NA'
	if type(inputs[10]) == np.float64:
		rand_err_for_sdss_g_flux = (random.uniform(-1,1))*inputs[10]
	else:
		rand_err_for_sdss_g_flux = 'NA'
	if type(inputs[12]) == np.float64:
		rand_err_for_sdss_r_flux = (random.uniform(-1,1))*inputs[12]
	else:
		rand_err_for_sdss_r_flux = 'NA'
	if type(inputs[14]) == np.float64:
		rand_err_for_sdss_i_flux = (random.uniform(-1,1))*inputs[14]
	else:
		rand_err_for_sdss_i_flux = 'NA'
	if type(inputs[16]) == np.float64:
		rand_err_for_sdss_z_flux = (random.uniform(-1,1))*inputs[16]
	else:
		rand_err_for_sdss_z_flux = 'NA'
	if type(inputs[18]) == np.float64:
		rand_err_for_DISTANCE = (random.uniform(-1,1))*inputs[18]
	else:
		rand_err_for_DISTANCE = 'NA'
	if type(inputs[20]) == np.float64:
		rand_err_for_W1_flux = (random.uniform(-1,1))*inputs[20]
	else:
		rand_err_for_W1_flux = 'NA'
	if type(inputs[22]) == np.float64:
		rand_err_for_W2_flux = (random.uniform(-1,1))*inputs[22]
	else:
		rand_err_for_W2_flux = 'NA'
	if type(inputs[24]) == np.float64:
		rand_err_for_W3_flux = (random.uniform(-1,1))*inputs[24]
	else:
		rand_err_for_W3_flux = 'NA'
	if type(inputs[26]) == np.float64:
		rand_err_for_W4_flux = (random.uniform(-1,1))*inputs[26]
	else:
		rand_err_for_W4_flux = 'NA'

	if type(inputs[28]) == np.float64:
		rand_err_for_FUV_flux = (random.uniform(-1,1))*inputs[28]
	else:
		rand_err_for_FUV_flux = 'NA'
	if type(inputs[30]) == np.float64:
		rand_err_for_NUV_flux = (random.uniform(-1,1))*inputs[30]
	else:
		rand_err_for_NUV_flux = 'NA'

	if type(inputs[34]) == np.float64:
		rand_err_for_J_flux_2MASS = (random.uniform(-1,1))*inputs[34]
	else:
		rand_err_for_J_flux_2MASS = 'NA'

	if type(inputs[36]) == np.float64:
		rand_err_for_H_flux_2MASS = (random.uniform(-1,1))*inputs[36]
	else:
		rand_err_for_H_flux_2MASS = 'NA'

	if type(inputs[38]) == np.float64:
		rand_err_for_K_flux_2MASS = (random.uniform(-1,1))*inputs[38]
	else:
		rand_err_for_K_flux_2MASS = 'NA'
	if type(inputs[40]) == np.float64:
		rand_err_for_Johnson_B_flux = (random.uniform(-1,1))*inputs[40]
	else:
		rand_err_for_Johnson_B_flux = 'NA'
	if type(inputs[42]) == np.float64:
		rand_err_for_Johnson_V_flux = (random.uniform(-1,1))*inputs[42]
	else:
		rand_err_for_Johnson_V_flux = 'NA'


	Teff = inputs[1]+rand_err_for_Teff					#apply random error to their respective parameter
	Teff = float(Teff)
	...
	Temperature_values.append(Teff)					#save each SET of parameters... 1 set is used per iteration of the Monte Carlo Simulation
...
def f(params):			#this is the SED fitting function
		...
		if max_temp == 160:						#This LONG portion of code finds the 1, 2, 3, or 4 closest models from the model grid to the "chosen" Teff and logg... we always want the 4 closest if we can, but default to 3, 2, or even 1 closest model when necessary
			temp_low = 16000.0
			temp_high = 16000.0
			t_low = str(160)
			t_high = str(160)
		...
		if Teff >= 50000.0:
			...
			if compare_grav<=LOGg:
				...
				#MODEL CONVOLUTION
		models_to_convolve = []				#convolution is a weighted average (weighted by how close the "chosen" teff and logg are to the model temps and gravities)
		...
		if len(models_to_convolve) == 2:					#here is where we apply the weighted average
		...
		for redwl in redlambdas:				#apply the wavelength-dependent interstellar reddening correction to the convolved model spectrum
		...
		reddened_convolved_model_spectrum = array([redlambdas,reddened_flux]).T
			...
			#SMOOTH REDDENED CONVOLVED MODEL SPECTRUM
		sigma = 2.1			#this corresponds to a fwhm of 5 angstroms which is approximately correct for SDSS spectra
		...
		flux_for_2MASS_K = []				#2MASS K filter had issues for some reason, so I just created its own (wavelength,flux) data from the model
		lam_for_2MASS_K = []
		...
		if type(params[18]) != str:				#create synthetic photometry (in erg/cm^2/s/angstrom)
			...
			for character in range(0,len(J_2MASS_string)):
				if J_2MASS_string[character] != ' ':
					J_2MASS_real_string.append(J_2MASS_string[character])
				else:
					break
			J_2MASS_realL_string = J_2MASS_real_string[0]
			for sp in range(1,len(J_2MASS_real_string)):
				J_2MASS_realL_string = J_2MASS_realL_string + J_2MASS_real_string[sp]
			J_2MASS_synthetic_flux_erg = np.float64(J_2MASS_realL_string)
			J2mass_check.append(J_2MASS_synthetic_flux_erg)				#this "check" feature will allow you to look at synthetic photometric values for each filter after competion of the Monte Carlo simulation
		if type(params[18]) == str:							#we do not create synthetic photometry for those filters which have NO observational error stated
			J_2MASS_synthetic_flux_erg = 'Not_Needed'
			J2mass_check.append(J_2MASS_synthetic_flux_erg)
		if type(params[19]) != str:
			...
			for character in range(0,len(H_2MASS_string)):
				if H_2MASS_string[character] != ' ':
					H_2MASS_real_string.append(H_2MASS_string[character])
				else:
					break
			H_2MASS_realL_string = H_2MASS_real_string[0]
			for sp in range(1,len(H_2MASS_real_string)):
				H_2MASS_realL_string = H_2MASS_realL_string + H_2MASS_real_string[sp]
			H_2MASS_synthetic_flux_erg = np.float64(H_2MASS_realL_string)
			H2mass_check.append(H_2MASS_synthetic_flux_erg)
		if type(params[19]) == str:
			H_2MASS_synthetic_flux_erg = 'Not_Needed'
			H2mass_check.append(H_2MASS_synthetic_flux_erg)
		if type(params[20]) != str:
			...
			for character in range(0,len(K_2MASS_string)):
				if K_2MASS_string[character] != ' ':
					K_2MASS_real_string.append(K_2MASS_string[character])
				else:
					break
			K_2MASS_realL_string = K_2MASS_real_string[0]
			for sp in range(1,len(K_2MASS_real_string)):
				K_2MASS_realL_string = K_2MASS_realL_string + K_2MASS_real_string[sp]
			K_2MASS_synthetic_flux_erg = np.float64(K_2MASS_realL_string)
			K2mass_check.append(K_2MASS_synthetic_flux_erg)
		if type(params[20]) == str:
			K_2MASS_synthetic_flux_erg = 'Not_Needed'
			K2mass_check.append(K_2MASS_synthetic_flux_erg)
		...
		if type(params[14]) != str:
			...
			Johnson_B_string = Johnson_B_temp_str[0]+Johnson_B_temp_str[1]+Johnson_B_temp_str[2]+Johnson_B_temp_str[3]+Johnson_B_temp_str[4]+Johnson_B_temp_str[5]+Johnson_B_temp_str[6]+Johnson_B_temp_str[7]+Johnson_B_temp_str[8]+Johnson_B_temp_str[9]+Johnson_B_temp_str[10]
			Johnson_B_real_string = []
			for character in range (0,len(Johnson_B_string)):
				if Johnson_B_string[character] != ' ':
					Johnson_B_real_string.append(Johnson_B_string[character])
				else:
					break
			Johnson_B_realL_string = Johnson_B_real_string[0]
			for sp in range(1,len(Johnson_B_real_string)):
				Johnson_B_realL_string = Johnson_B_realL_string + Johnson_B_real_string[sp]
			Johnson_B_synthetic_flux_erg = np.float64(Johnson_B_realL_string)
			JB_check.append(Johnson_B_synthetic_flux_erg)
		if type(params[14]) == str:
			Johnson_B_synthetic_flux_erg = 'Not_Needed'
			JB_check.append(Johnson_B_synthetic_flux_erg)
		if type(params[15]) != str:
			...
			Johnson_V_string = Johnson_V_temp_str[0]+Johnson_V_temp_str[1]+Johnson_V_temp_str[2]+Johnson_V_temp_str[3]+Johnson_V_temp_str[4]+Johnson_V_temp_str[5]+Johnson_V_temp_str[6]+Johnson_V_temp_str[7]+Johnson_V_temp_str[8]+Johnson_V_temp_str[9]+Johnson_V_temp_str[10]
			Johnson_V_real_string = []
			for character in range (0,len(Johnson_V_string)):
				if Johnson_V_string[character] != ' ':
					Johnson_V_real_string.append(Johnson_V_string[character])
				else:
					break
			Johnson_V_realL_string = Johnson_V_real_string[0]
			for sp in range(1,len(Johnson_V_real_string)):
				Johnson_V_realL_string = Johnson_V_realL_string + Johnson_V_real_string[sp]
			Johnson_V_synthetic_flux_erg = np.float64(Johnson_V_realL_string)
			JV_check.append(Johnson_V_synthetic_flux_erg)
		if type(params[15]) == str:
			Johnson_V_synthetic_flux_erg = 'Not_Needed'
			JV_check.append(Johnson_V_synthetic_flux_erg)
		if type(params[5]) != str:
			...
			for character in range (0,len(sdss_u_string)):
				if sdss_u_string[character] != ' ':
					sdss_u_real_string.append(sdss_u_string[character])
				else:
					break
			sdss_u_realL_string = sdss_u_real_string[0]
			for sp in range(1,len(sdss_u_real_string)):
				sdss_u_realL_string = sdss_u_realL_string + sdss_u_real_string[sp]
			sdss_u_synthetic_flux_erg = np.float64(sdss_u_realL_string)
			sdssu_check.append(sdss_u_synthetic_flux_erg)
		if type(params[5]) == str:
			sdss_u_synthetic_flux_erg = 'Not_Needed'
			sdssu_check.append(sdss_u_synthetic_flux_erg)
		if type(params[6]) != str:
			...
			for character in range (0,len(sdss_g_string)):
				if sdss_g_string[character] != ' ':
					sdss_g_real_string.append(sdss_g_string[character])
				else:
					break
			sdss_g_realL_string = sdss_g_real_string[0]
			for sp in range(1,len(sdss_g_real_string)):
				sdss_g_realL_string = sdss_g_realL_string + sdss_g_real_string[sp]
			sdss_g_synthetic_flux_erg = np.float64(sdss_g_realL_string)
			sdssg_check.append(sdss_g_synthetic_flux_erg)
		if type(params[6]) == str:
			sdss_g_synthetic_flux_erg = 'Not_Needed'
			sdssg_check.append(sdss_g_synthetic_flux_erg)
		if type(params[7]) != str:
			...
			for character in range (0,len(sdss_r_string)):
				if sdss_r_string[character] != ' ':
					sdss_r_real_string.append(sdss_r_string[character])
				else:
					break
			sdss_r_realL_string = sdss_r_real_string[0]
			for sp in range(1,len(sdss_r_real_string)):
				sdss_r_realL_string = sdss_r_realL_string + sdss_r_real_string[sp]
			sdss_r_synthetic_flux_erg = np.float64(sdss_r_realL_string)
			sdssr_check.append(sdss_r_synthetic_flux_erg)
		if type(params[7]) == str:
			sdss_r_synthetic_flux_erg = 'Not_Needed'
			sdssr_check.append(sdss_r_synthetic_flux_erg)
		if type(params[8]) != str:
			...
			for character in range (0,len(sdss_i_string)):
				if sdss_i_string[character] != ' ':
					sdss_i_real_string.append(sdss_i_string[character])
				else:
					break
			sdss_i_realL_string = sdss_i_real_string[0]
			for sp in range(1,len(sdss_i_real_string)):
				sdss_i_realL_string = sdss_i_realL_string + sdss_i_real_string[sp]
			sdss_i_synthetic_flux_erg = np.float64(sdss_i_realL_string)
			sdssi_check.append(sdss_i_synthetic_flux_erg)
		if type(params[8]) == str:
			sdss_i_synthetic_flux_erg = 'Not_Needed'
			sdssi_check.append(sdss_i_synthetic_flux_erg)
		if type(params[9]) != str:
			...
			for character in range (0,len(sdss_z_string)):
				if sdss_z_string[character] != ' ':
					sdss_z_real_string.append(sdss_z_string[character])
				else:
					break
			sdss_z_realL_string = sdss_z_real_string[0]
			for sp in range(1,len(sdss_z_real_string)):
				sdss_z_realL_string = sdss_z_realL_string + sdss_z_real_string[sp]
			sdss_z_synthetic_flux_erg = np.float64(sdss_z_realL_string)
			sdssz_check.append(sdss_z_synthetic_flux_erg)
		if type(params[9]) == str:
			sdss_z_synthetic_flux_erg = 'Not_Needed'
			sdssz_check.append(sdss_z_synthetic_flux_erg)
		if type(params[10]) != str:
			...
			WISE1_string = WISE1_temp_str[0]+WISE1_temp_str[1]+WISE1_temp_str[2]+WISE1_temp_str[3]+WISE1_temp_str[4]+WISE1_temp_str[5]+WISE1_temp_str[6]+WISE1_temp_str[7]+WISE1_temp_str[8]+WISE1_temp_str[9]
			WISE1_real_string = []
			for character in range (0,len(WISE1_string)):
				if WISE1_string[character] != ' ':
					WISE1_real_string.append(WISE1_string[character])
				else:
					break
			WISE1_realL_string = WISE1_real_string[0]
			for sp in range(1,len(WISE1_real_string)):
				WISE1_realL_string = WISE1_realL_string + WISE1_real_string[sp]
			WISE1_synthetic_flux_erg = np.float64(WISE1_realL_string)
			W1_check.append(WISE1_synthetic_flux_erg)
		if type(params[10]) == str:
			WISE1_synthetic_flux_erg = 'Not_Needed'
			W1_check.append(WISE1_synthetic_flux_erg)
		if type(params[11]) != str:
			...
			WISE2_string = WISE2_temp_str[0]+WISE2_temp_str[1]+WISE2_temp_str[2]+WISE2_temp_str[3]+WISE2_temp_str[4]+WISE2_temp_str[5]+WISE2_temp_str[6]+WISE2_temp_str[7]+WISE2_temp_str[8]+WISE2_temp_str[9]
			WISE2_real_string = []
			for character in range (0,len(WISE2_string)):
				if WISE2_string[character] != ' ':
					WISE2_real_string.append(WISE2_string[character])
				else:
					break
			WISE2_realL_string = WISE2_real_string[0]
			for sp in range(1,len(WISE2_real_string)):
				WISE2_realL_string = WISE2_realL_string + WISE2_real_string[sp]
			WISE2_synthetic_flux_erg = np.float64(WISE2_realL_string)
			W2_check.append(WISE2_synthetic_flux_erg)
		if type(params[11]) == str:
			WISE2_synthetic_flux_erg = 'Not_Needed'
			W2_check.append(WISE2_synthetic_flux_erg)
		if type(params[12]) != str:
			...
			WISE3_string = WISE3_temp_str[0]+WISE3_temp_str[1]+WISE3_temp_str[2]+WISE3_temp_str[3]+WISE3_temp_str[4]+WISE3_temp_str[5]+WISE3_temp_str[6]+WISE3_temp_str[7]+WISE3_temp_str[8]
			WISE3_real_string = []
			for character in range (0,len(WISE3_string)):
				if WISE3_string[character] != ' ':
					WISE3_real_string.append(WISE3_string[character])
				else:
					break
			WISE3_realL_string = WISE3_real_string[0]
			for sp in range(1,len(WISE3_real_string)):
				WISE3_realL_string = WISE3_realL_string + WISE3_real_string[sp]
			WISE3_synthetic_flux_erg = np.float64(WISE3_realL_string)
			W3_check.append(WISE3_synthetic_flux_erg)
		if type(params[12]) == str:
			WISE3_synthetic_flux_erg = 'Not_Needed'
			W3_check.append(WISE3_synthetic_flux_erg)
		if type(params[13]) != str:
			...
			WISE4_string = WISE4_temp_str[0]+WISE4_temp_str[1]+WISE4_temp_str[2]+WISE4_temp_str[3]+WISE4_temp_str[4]+WISE4_temp_str[5]+WISE4_temp_str[6]+WISE4_temp_str[7]+WISE4_temp_str[8]
			WISE4_real_string = []
			for character in range (0,len(WISE4_string)):
				if WISE4_string[character] != ' ':
					WISE4_real_string.append(WISE4_string[character])
				else:
					break
			WISE4_realL_string = WISE4_real_string[0]
			for sp in range(1,len(WISE4_real_string)):
				WISE4_realL_string = WISE4_realL_string + WISE4_real_string[sp]
			WISE4_synthetic_flux_erg = np.float64(WISE4_realL_string)
			W4_check.append(WISE4_synthetic_flux_erg)
		if type(params[13]) == str:
			WISE4_synthetic_flux_erg = 'Not_Needed'
			W4_check.append(WISE4_synthetic_flux_erg)
		if type(params[16]) != str:
			...
			fuv_galex_string = fuv_galex_temp_str[0]+fuv_galex_temp_str[1]+fuv_galex_temp_str[2]+fuv_galex_temp_str[3]+fuv_galex_temp_str[4]+fuv_galex_temp_str[5]+fuv_galex_temp_str[6]+fuv_galex_temp_str[7]+fuv_galex_temp_str[8]+fuv_galex_temp_str[9]+fuv_galex_temp_str[10]+fuv_galex_temp_str[11]+fuv_galex_temp_str[12]+fuv_galex_temp_str[13]
			fuv_galex_real_string = []
			for character in range (0,len(fuv_galex_string)):
				if fuv_galex_string[character] != ' ':
					fuv_galex_real_string.append(fuv_galex_string[character])
				else:
					break
			fuv_galex_realL_string = fuv_galex_real_string[0]
			for sp in range(1,len(fuv_galex_real_string)):
				fuv_galex_realL_string = fuv_galex_realL_string + fuv_galex_real_string[sp]
			fuv_galex_synthetic_flux_erg = np.float64(fuv_galex_realL_string)
			fuv_check.append(fuv_galex_synthetic_flux_erg)
		if type(params[16]) == str:
			fuv_galex_synthetic_flux_erg = 'Not_Needed'
			fuv_check.append(fuv_galex_synthetic_flux_erg)
		if type(params[17]) != str:
			...
			for character in range (0,len(nuv_galex_string)):
				if nuv_galex_string[character] != ' ':
					nuv_galex_real_string.append(nuv_galex_string[character])
				else:
					break
			nuv_galex_realL_string = nuv_galex_real_string[0]
			for sp in range(1,len(nuv_galex_real_string)):
				nuv_galex_realL_string = nuv_galex_realL_string + nuv_galex_real_string[sp]
			nuv_galex_synthetic_flux_erg = np.float64(nuv_galex_realL_string)
			nuv_check.append(nuv_galex_synthetic_flux_erg)
		if type(params[17]) == str:
			nuv_galex_synthetic_flux_erg = 'Not_Needed'
			nuv_check.append(nuv_galex_synthetic_flux_erg)


			#(observed) Relative Errors for filters
		total_relative_err_to_sum = []					#the size of each filter's observational error relative to its phtometric value determines its weight (to be used in weighted flux ratio and weighted chi square calcuations)
		...
		if type(J_2MASS_synthetic_flux_erg) != str:				#find flux ratio for each filter, then average
		...
		avg_flux_ratio = np.sum(to_average_fr)/len(to_average_fr)		#this is theta^2 (theta^2 = R^2/distance^2)
		...
		if type(fuv_galex_synthetic_flux_erg) != str:				#calculate weighted chi square and aboslute error
		...
		MASS_parameters_then_data = [chi_square,params[1],radius_in_solar_radii,params[0],params[2],params[3],params[4],radius_error_in_solar_radii,filters_used,absolute_error,fuv_check,nuv_check,sdssu_check,sdssg_check,sdssr_check,sdssi_check,sdssz_check,JB_check,JV_check,J2mass_check,H2mass_check,K2mass_check,W1_check,W2_check,W3_check,W4_check]
		return(MASS_parameters_then_data)

if __name__ == '__main__':			#parallelization... you need to check how many cores your CPU has and use 1 less than the maximum. My computer has 8 cores so I use 7 of them (using 8 dramatically increases runtime)
        with Pool(7) as p:
...
for list in results:
	...
	fuv_CHECK.append(list[10])
	nuv_CHECK.append(list[11])
	sdssu_CHECK.append(list[12])
	sdssg_CHECK.append(list[13])
	sdssr_CHECK.append(list[14])
	sdssi_CHECK.append(list[15])
	sdssz_CHECK.append(list[16])
	JohnsonB_CHECK.append(list[17])
	JohnsonV_CHECK.append(list[18])
	J2mass_CHECK.append(list[19])
	H2mass_CHECK.append(list[20])
	K2mass_CHECK.append(list[21])
	W1_CHECK.append(list[22])
	W2_CHECK.append(list[23])
	W3_CHECK.append(list[24])
	W4_CHECK.append(list[25])

for i in range(len(chi_square_error)):					#mass caluclation using the logg and radius values corresponding to lowest chi square run
...
os.chdir('/media/intern2/DD82-B92F/RESEARCH_FALL_2022/saved_sed_values/')		#change to a directory where you want to save all output values

with open(star_name+'.txt', "w") as f:		#this saves all sed parameters and results. It will name each file based on star_name, and will override data if a star_name.txt file already exists
...
with open(star_name+'_observed_params.txt', 'w') as f:		#this saves teff w/ err, logg w/ err, reddening parameters w/ errors, and distance w/ error (these are the initial values and error budgets we vary around in the Monte Carlo simulation)
...
with open(star_name+'_CHECK', "w") as f:			#this saves synthetic photometry for all filters used
        for i in range(0,len(fuv_CHECK)):
                f.write(str(results[i][10][0])+'\t'+str(results[i][11][0])+'\t'+str(results[i][12][0])+'\t'+str(results[i][13][0])+'\t'+str(results[i][14][0])+'\t'+str(results[i][15][0])+'\t'+str(results[i][16][0])+'\t'+str(results[i][17][0])+'\t'+str(results[i][18][0])+'\t'+str(results[i][19][0])+'\t'+str(results[i][20][0])+'\t'+str(results[i][21][0])+'\t'+str(results[i][22][0])+'\t'+str(results[i][23][0])+'\t'+str(results[i][24][0])+'\t'+str(results[i][25][0])+'\n')



					#ABSOLUTE ERROR VS FILTER PLOT
